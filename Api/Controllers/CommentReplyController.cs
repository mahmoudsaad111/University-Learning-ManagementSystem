...
namespace Api.Controllers
{
    ...
    public class CommentReplyController : ControllerBase
    {
        ...
        public async Task<ActionResult> DeleteCommentReply([FromHeader] int Id, [FromBody] CommentReplyDto commentReplyDto)
        {
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            if (Id == 0)
            ...
            try
            {
                Result<int> resultOfDeleted = await mediator.Send(new DeleteCommentReplyCommand { Id = Id, CommentReplyDto = commentReplyDto });
